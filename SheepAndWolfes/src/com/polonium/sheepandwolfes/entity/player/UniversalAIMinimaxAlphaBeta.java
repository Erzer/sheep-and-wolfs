...
public class UniversalAIMinimaxAlphaBeta implements Player {
    ...
    public UniversalAIMinimaxAlphaBeta(int deep) {
        this.deep = deep;
    }

    @Override
    public UniversalAIMinimaxAlphaBeta makeMove(GameState gameState, OnMakeMoveListener listener) {
    ...
    class Thinker extends AsyncTask<Void, Void, GameState> {
        ...
        protected GameState doInBackground(Void... params) {
            ...
            return minimax(currentState, (deep - 1) * 2 + 1, 0, 255);
        }

        private GameState minimax(GameState gs, int recursiveLevel, int alpha, int beta) {
            ...
            if (gs.lastMove == GameState.WOLFS) {
                ...
                for (GameState possible : sheepPosibleMoves) {
                    if (beta < alpha) break;
                    int minimax = minimax(possible, recursiveLevel - 1, alpha, beta).rate;
                    if (minimax <= best || bestGameState == null) {
                        best = minimax;
                        bestGameState = possible;
                        bestGameState.rate = minimax;
                    }
        ...
        protected void onPostExecute(GameState result) {
            if (!mGameOver) makeMoveListener.onMoveComlete(UniversalAIMinimaxAlphaBeta.this, result);
        }
